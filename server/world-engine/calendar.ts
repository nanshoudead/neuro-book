...
const CALENDAR_RELATIVE_PATH = "world-engine/calendar.yaml";
...
// YAML 是外部输入，进入 normalizeCalendarConfig 前只能以 unknown 边界读取。
type RawCalendarConfig = Record<string, unknown>;

/** Project Calendar：负责人读时间字符串与 Instant 的双向转换。 */
export class WorldCalendar {
    ...
    private instantFromParts(parts: {year: bigint; month: bigint; day: bigint; hour: bigint; minute: bigint; second: bigint}): Instant {
        ...
        const secondsPerMonth = secondsPerDay * this.config.daysPerMonth;
        const secondsPerYear = secondsPerMonth * this.config.monthsPerYear;
        return (parts.year - ONE) * secondsPerYear
            + (parts.month - ONE) * secondsPerMonth
            + (parts.day - ONE) * secondsPerDay
            + parts.hour * secondsPerHour
            + parts.minute * this.config.secondsPerMinute
            + parts.second;
    }
...
/** 读取项目日历配置；不存在时使用默认 12x30 奇幻简明历。 */
export class WorldCalendarLoader {
    async load(projectPath: string): Promise<WorldCalendar> {
        const calendarPath = path.join(resolveProjectAbsolutePath(projectPath), CALENDAR_RELATIVE_PATH);
        try {
            const parsed = yaml.parse(await fs.readFile(calendarPath, "utf-8")) as unknown;
            return new WorldCalendar(normalizeCalendarConfig(parsed));
        } catch (error) {
            if (typeof error === "object" && error !== null && "code" in error && error.code === "ENOENT") {
                return new WorldCalendar(normalizeCalendarConfig(null));
            }
            throw createError({statusCode: 400, message: `世界日历解析失败：${error instanceof Error ? error.message : String(error)}`});
        }
    }
}

function normalizeCalendarConfig(input: unknown): CalendarConfig {
