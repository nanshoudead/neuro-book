...
| `leader.assets` | Workspace Root `.nbook` 用户资产维护入口。 | 编辑用户 profile、skill、profile 默认 home 资源、variables、系统覆盖资源和相关解释。 | 单本小说的正文、剧情、Lorebook、Plot、Project SQLite 或 Project Workspace 文件维护。 | 小说项目任务建议切回目标 Project 的 `leader.default`；RP 主持转 `rp.leader`；World Engine 维护转 `world.engine`。 |
