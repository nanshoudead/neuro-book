...
2. **自取 / 消费 brief**：
   - 有 `input.chapterId` 时用 `get_chapter_writer_brief` 自取;需要场景与线索用 `get_chapter_plot` / `get_story_scene_context`;剧情设计权仍在 leader，你**只读 Plot，不创建/修改** Thread / Scene / Chapter。
