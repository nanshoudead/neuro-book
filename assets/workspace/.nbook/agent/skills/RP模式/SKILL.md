...
- 当前 Project Workspace 应存在 `manual/README.md`、`manual/player-guide/README.md`、`manual/player-guide/character-creation.md`、`manual/gm-guide.md`、`agent-context/rp.leader/context.md`、`agent-context/simulator.leader/context.md` 与 `simulation/subjects/*`；`agent-context/rp.writer/context.md` 可作为上级构造 writer brief 时读取的写作偏好，不是 `rp.writer` 的 profile input。
...
- `rp.writer` profile input 为空，只消费上级注入的 writer brief。若需要使用 `agent-context/rp.writer/context.md` 中的写作偏好，由 `rp.leader` 或 `simulator.leader` 先读取并写进 brief；`rp.writer` 自身不自主遍历 `simulation/`、`lorebook/`、`manual/` 或 `agent-context/`。
...
- `manual/`、`agent-context/rp.leader/context.md`、`agent-context/simulator.leader/context.md` 和 `simulation/` runtime 目录存在；如项目维护 `agent-context/rp.writer/context.md`，上级知道要把其中可写偏好注入 writer brief。
