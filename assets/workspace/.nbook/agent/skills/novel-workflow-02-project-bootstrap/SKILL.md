...
- 留下初步剧情大纲构思和开局剧情构思，方便后续 `novel-workflow-03-lorebook-bootstrap`、`novel-workflow-05-emulation-bootstrap` / `novel-workflow-06-emulation-tick` 和 `novel-workflow-07-opening-plot-design`。
...
如果用户已经要求写开篇，切到 `novel-workflow-07-opening-plot-design`。如果用户要补规则、势力、种族、地点或力量体系，切到 `novel-workflow-03-lorebook-bootstrap`。如果用户要排中长期剧情，切到 `novel-workflow-08-plot-planning`。
...
- 下一步建议：通常先进入 `novel-workflow-03-lorebook-bootstrap`，再按需要进入 `novel-workflow-05-emulation-bootstrap` / `novel-workflow-06-emulation-tick`，最后进入 `novel-workflow-07-opening-plot-design`。
