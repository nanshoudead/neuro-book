...
const canRename = computed(() => !props.disabled && Boolean(selectedKey.value) && Boolean(resource.value?.capabilities.rename));
...
function createResource(): void {
    ...
    if (!label || !resource.value?.capabilities.create) return;
    ...
    upsertMutation({
        ...
        label,
        slug,
        content: resource.value.template ?? "",
    });
...
function renameResource(): void {
    ...
    if (!label || !selectedKey.value || !resource.value?.capabilities.rename) return;
    const slug = uniqueSlug(label);
    upsertMutation({
        type: "rename",
        fieldPath: props.field.path,
...
function uniqueSlug(label: string): string {
    ...
    const usedKeys = new Set(visibleOptions.value.map((option) => option.key));
        ...
        <template v-else>
            ...
            <div class="overflow-visible rounded-md border border-[var(--border-color)] bg-[var(--bg-input)]/25">
                <div class="flex min-w-0 items-center gap-2 px-2 py-2">
                    ...
                    <button v-if="canManage || selectedKey" type="button" class="inline-flex h-7 shrink-0 items-center gap-1 rounded-md border border-[var(--border-color)] px-2 text-[11px] text-[var(--text-main)] hover:bg-[var(--bg-hover)] disabled:opacity-50" :disabled="!selectedKey && !canManage" @click="managerOpen = !managerOpen">
                ...
                <div v-if="managerOpen" class="border-t border-[var(--border-color)]">
                    ...
                    <div class="min-w-0">
                        ...
                        <div v-if="selectedKey" class="flex h-full p-2">
                            <textarea
                                class="min-h-[150px] w-full flex-1 resize-y overflow-auto rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] p-3 font-mono text-xs leading-relaxed text-[var(--text-main)] outline-none focus:border-[var(--primary)] disabled:opacity-60"
