...
<template>
    ...
    <div class="space-y-3">
        ...
        <div class="flex flex-wrap items-center gap-2">
            <button type="button" class="inline-flex h-9 items-center gap-2 rounded-md border border-[var(--border-color)] px-4 text-[13px] text-[var(--text-main)] hover:bg-[var(--bg-hover)] disabled:opacity-50" :disabled="!canSubmit" @click="emit('submit')">
                <span :class="saving ? 'i-lucide-loader-2 animate-spin' : editingSliceId ? 'i-lucide-save' : 'i-lucide-send'" class="h-4 w-4"></span>
                {{ actionLabel }}
            </button>
            <button v-if="showContinueAction" type="button" class="inline-flex h-9 items-center gap-2 rounded-md border border-[var(--accent-main)]/40 px-4 text-[13px] text-[var(--accent-main)] hover:bg-[var(--accent-bg)] disabled:opacity-50" :disabled="!canSubmit" @click="emit('submit-and-continue')">
