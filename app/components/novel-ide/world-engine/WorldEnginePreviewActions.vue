...
const canCreateSubject = computed(() => props.projectReady && !props.actionBusy && props.subjectForm.id.trim() && props.subjectForm.type.trim() && props.subjectForm.time.trim() && !subjectIdAlreadyExists.value);
const canWriteSlice = computed(() => props.projectReady && !props.actionBusy && props.sliceForm.time.trim());
</script>
    ...
    <section class="min-w-0 rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)]">
        ...
        <div class="space-y-5 p-4">
            ...
            <div class="space-y-2 border-t border-[var(--border-color)] pt-5">
                ...
                <button type="button" class="inline-flex h-9 w-full items-center justify-center gap-2 rounded-md border border-[var(--border-color)] px-3 text-sm hover:bg-[var(--bg-hover)] disabled:opacity-50" :disabled="!projectReady || actionBusy" @click="emit('query-state')">
