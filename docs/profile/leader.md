...
`leader.rp` 是世界模拟 / RP 的 simulator leader。它读取 `simulation/config.yaml`、`simulation/cast.yaml`、`simulation/simulator.md` 和 `simulation/writer.md`，并调度 `simulator.actor` 与 `rp.writer`。

一次 RP Tick 通常是：

```text
用户行动 -> leader.rp 裁决和调度 -> simulator.actor 角色反应 -> leader.rp 世界推进 -> rp.writer 渲染可见正文
```
