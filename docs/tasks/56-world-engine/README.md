...
- 默认 Project Workspace 模板已包含 `world-engine/schema.yaml` 与 `world-engine/calendar.yaml`，新建 Project 可直接使用 World Engine。
...
- 独立 `/world-engine.preview` 的 Schema / Calendar 路径 chip 已可打开主 IDE 深链：`/?project=<projectPath>&openPath=world-engine/schema.yaml|world-engine/calendar.yaml`；主 IDE 会在 Project route 同步完成后打开对应 Project Workspace 文件，补上新建 Project 后设置 subject schema / calendar 的直达路径。
...
- 第二百四十三轮收口 `knownSliceTimes` 合并顺序：局部 / 懒加载旧切片时间只补到已知窗口前方，不抢走完整 timeline 末尾的默认下一时间基准。
...
- 独立 Preview 后续若继续扩展 UI，建议优先放进已拆出的子组件；当前 `world-engine.preview.vue` 约 751 行，仍低于 800 行限制，但不要继续把复杂 Builder 逻辑堆回页面单文件。
