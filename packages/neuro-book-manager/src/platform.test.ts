...
describe("Manager平台矩阵", () => {
    ...
    it("POSIX平台支持除Windows Portable外的全部Profile", () => {
        for (const platform of PLATFORMS.filter((item) => item !== "windows-x64")) {
    ...
    it("每个平台都有唯一公开Product资产名", () => {
        expect(Object.keys(PRODUCT_ASSET_NAMES)).toEqual(PLATFORMS);
        expect(new Set(Object.values(PRODUCT_ASSET_NAMES)).size).toBe(PLATFORMS.length);
    });

    it("Managed Bun和ripgrep对五个平台都有明确资产", () => {
        expect(Object.keys(BUN_ASSET_NAMES)).toEqual(PLATFORMS);
        expect(Object.keys(RIPGREP_ASSET_SUFFIXES)).toEqual(PLATFORMS);
